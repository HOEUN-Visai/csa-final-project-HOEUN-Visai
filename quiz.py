...
def loginPage(logdata):
    ...
    uname = Entry(login_frame, bg='#d3d3d3',
                  fg='white', textvariable=user_name)
...
#        L2.config(text="Username is "+z[0][0]+"\nPassword is "+z[-1][1])
    ...
    sp = Button(sup_frame, text='SignUp', padx=5, pady=5,
                width=5, command=addUserToDataBase)
    sp.configure(width=15, height=1, activebackground="#33B5E5", relief=FLAT)
    sp.place(relx=0.4, rely=0.8)

    log = Button(sup_frame, text='Already have a Account?', padx=5,
                 pady=5, width=5, command=gotoLogin, bg="white")
...
def menu():
    ...
    var = IntVar()
    easy = Radiobutton(menu_frame, text='Easy', bg="white",
                       font="calibri 16", value=1, variable=var)
    easy.place(relx=0.25, rely=0.4)

    medium = Radiobutton(menu_frame, text='Medium', bg="white",
                         font="calibri 16", value=2, variable=var)
    medium.place(relx=0.25, rely=0.5)

    hard = Radiobutton(menu_frame, text='Hard', bg="white",
...
def easy():
    ...
    var = StringVar()
    a = Radiobutton(
        easy_frame, text=easyQ[x][1], font="calibri 10", value=1, variable=var, bg="white")
    a.place(relx=0.5, rely=0.42, anchor=CENTER)

    b = Radiobutton(
        easy_frame, text=easyQ[x][2], font="calibri 10", value=2, variable=var, bg="white")
    b.place(relx=0.5, rely=0.52, anchor=CENTER)

    c = Radiobutton(
        easy_frame, text=easyQ[x][3], font="calibri 10", value=3, variable=var, bg="white")
    c.place(relx=0.5, rely=0.62, anchor=CENTER)

    d = Radiobutton(
        easy_frame, text=easyQ[x][4], font="calibri 10", value=4, variable=var, bg="white")
